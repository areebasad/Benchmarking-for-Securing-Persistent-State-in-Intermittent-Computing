...
************************************************************************
 ...
 Description: Encryption/Decryption using AES module (Hardware). 
	...
	by each task encrypt, decrypt, writing on flash and reading from flash.
...
void aes_HW_measurement(void)
{
	...
	for (size_t num_bytes = MIN_NUM_BYTES; num_bytes <= MAX_NUM_BYTES; num_bytes += STEP_SIZE) {
		...
		/* Note: In file hpl_aes.c 
			Data type block variable changed from uint8 to uint32 to encrypt/decrypt input (data) of more than 4080 bytes.
		...
		*/
		...
		for (size_t count = 0;  count < num_bytes/STEP_SIZE; count++) {
			...
			//aes_sync_cbc_crypt(&CRYPTOGRAPHY_0, AES_ENCRYPT, input + (count*STEP_SIZE), input + (count*STEP_SIZE), STEP_SIZE, iv);  (CBC Mode, avoid loops)
		}
		...
		if (_rww_flash_write(&FLASH_0.dev, NVMCTRL_RWW_EEPROM_ADDR, input, num_bytes ) != ERR_NONE) {
			while (1); /* Trap here when flash write error happen */
		}
